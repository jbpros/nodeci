...
NodeciServer.prototype.initBuilder = function() {
  var self = this;
  self.builder = new Builder(self.settings.buildCommand, self.settings.buildArgs);
...
NodeciServer.prototype.isReady = function() {
  var self = this;
  return (self.builder.isReady() && !self.preparing);
};

NodeciServer.prototype.prepare = function() {
  var self = this;
  self.preparing = true;
  console.log('*** Preparing ***');
  var prepare = new Build(self.settings.prepareCommand, self.settings.prepareArgs);
...
NodeciServer.prototype.checkPrepareQueue = function() {
  var self = this;
  if (self.waitingForPrepare) {
    self.waitingForPrepare = false;
    self.prepare();
  }
};
