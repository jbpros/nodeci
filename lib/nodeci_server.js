...
function NodeciServer(options) {
  ...
  var self = this;
  var option_or_default = function(value, default_v) {
  ...
  if (self.settings.build_autostart) {
    self.builder.start(self.settings.build_continuous);
  }
};
...
NodeciServer.prototype.initBuilder = function() {
  ...
  self.builder.on('end', function(build) {
    self.bayeux_server.getClient().publish('/build-end', {
      code:       build.getCode(),
      signal:     build.getSignal(),
      succeeded:  build.didSucceed(),
