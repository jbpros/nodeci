...
NodeciServer.prototype.initBuilder = function() {
  ...
  self.builder.on('success', function() {
    self.bayeux_server.getClient().publish('/status', { finished: true, succeeded: true });
  });
  self.builder.on('failure', function(code, signal) {
    self.bayeux_server.getClient().publish('/status', { finished: true, succeeded: false });
  });
  self.builder.on('start', function() {
    self.bayeux_server.getClient().publish('/status', { finished: false, succeeded: false });
  });
